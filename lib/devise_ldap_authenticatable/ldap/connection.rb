    ...
    class Connection
      ...
      def initialize(params = {})
        if ::Devise.ldap_config.is_a?(Proc)
          ...
          ldap_config = YAML.load(ERB.new(File.read(::Devise.ldap_config || "#{Rails.root}/config/ldap.yml")).result)[Rails.env][params[:domain]]
        end

        raise 'Unable to load LDAP config' unless ldap_config
